...
interface VersionCmdStepModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const VersionCmdStepModal: React.FC<VersionCmdStepModalProps> =
  React.memo(({ isOpen, onClose, onSave, editingStep }) => {
    ...
    useEffect(() => {
      ...
      if (editingStep) {
        ...
        // Set mode based on command complexity
        setMode(
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto border-gray-700 bg-gray-800 text-white">
          <DialogHeader>
            <DialogTitle>
              {editingStep ? "Edit" : "Add"} Version Command
            </DialogTitle>
            <DialogDescription className="text-gray-400">
              Execute a command and extract version information
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            {mode === "advanced" && (
              <div className="space-y-2">
                  ...
                  {formData.command.map((arg, index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      <Input
                        value={arg}
                        onChange={(e) =>
                          handleUpdateArgument(index, e.target.value)
                        }
                ...
                <p className="text-xs text-gray-400">
                  Each argument is a separate field. Useful for arguments with
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-gray-400">
                Regular expression to extract version from output. Use capture
                groups to extract specific parts.
              </p>
              {errors.regex && (
                <p className="text-xs text-red-400">{errors.regex}</p>
              )}
            </div>
...
VersionCmdStepModal.displayName = "VersionCmdStepModal";
