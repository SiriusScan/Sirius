...
main() {
    ...
    run_test "Logging API Key Contract" "python3 -c \"from pathlib import Path; import re; api_handler = Path('sirius-api/handlers/log_handler.go').read_text(); engine_dockerfile = Path('sirius-engine/Dockerfile').read_text(); assert 'infraauth.LoadSiriusAPIKey' in api_handler; assert re.search(r'ARG GO_API_COMMIT_SHA=[0-9a-f]{40}', engine_dockerfile), 'engine Dockerfile must pin full GO_API_COMMIT_SHA'; assert 'git apply' not in engine_dockerfile\""
